...
本项目不局限于 Markdown 样式转换，也不采用缺乏过程可追溯性的默认自动发布流程。总体工作流如下：
...
- **内容契约前置**：每篇文章保存 `brief.yaml`、`claims.yaml`、`sources.yaml`、`draft.md`、
  `article.md` 和 `review-report.json`，确保内容决策过程可追溯。
- **个人材料真实性约束**：用户未提供个人材料时，可以表达作者判断，但不得虚构朋友、采访、
  对话、时间地点或感官场景。
- **编辑判断优先**：准确、观点、有用、合声、好读五项决定能否交付；自然表达度、SEO 和节奏
  指标仅用于风险提示，不用于判断作者身份。
- **Preview / Publish 分目标检查**：本地预览不要求微信密钥、封面或图片 API；发布则必须满足
  审稿、封面、图片、配置、HTML、编码和平台约束。
- **外部副作用显式授权**：“写一篇”和“完整制作”均不会创建微信草稿；仅在用户于当前会话
  明确要求“推到草稿箱 / 发布”后执行上传和草稿操作。
- **既有能力延续**：单篇独立目录、中文视觉提示词、稳定图片槽位、16 套主题、自动版式、
...
| 用户指令 | 默认输出 | 不执行的操作 |
|---|---|---|
| “写一篇公众号文章” | 通过编辑复审的成稿 + 本地预览 | 不生成图片、不发布 |
...
通过条件：平均分至少 4、单项不低于 3、没有阻断问题。可修复问题必须落实到正文并重新复审；
不得仅提供改进建议而将未经修改的初稿标记为成稿。
...
- `advisories`：自然表达度、SEO、可选增强和未请求能力的提示，不计入当前目标的阻断条件。

因此，“零 warning”是**目标相关的零 warning**：本地预览不需要为了清零去配置微信密钥，
纯文字文章也不需要为满足形式指标而强制添加正文图片；但发布必须通过审稿、摘要、双封面、
引用图片、微信配置、结构、编码、对比度和预检。
...
memorability、conversion；不以增加视觉复杂度为目的堆叠卡片。
